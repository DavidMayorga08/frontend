    ...
    <div class="q-pa-md">
      ...
      >
        <template v-slot:top-right>
          ...
          <q-select
            v-model="filtroTipo"
            :options="['Disponible', 'Agotado', 'Reservado']"
...
let columns = [
  ...
  },
  {
    ...
    align: "center",
    headerStyle: "font-weight: bold;",
    field: (row) => row.Fecha_expiracion.split("T")[0],
  },
  {
    name: "Id_proveedor",
    label: "Proveedor",
    ...
    field: (row) => {
      ...
      if (proveedor.Tipo_proveedor === "Natural") {
        return Array.isArray(proveedor.Datos_natural) && proveedor.Datos_natural[0]?.Nombre ? proveedor.Datos_natural[0].Nombre : "";
      } else {
        return Array.isArray(proveedor.Datos_juridico) &&
          Array.isArray(proveedor.Datos_juridico[0]?.Representante_legal) &&
...
let traerInsumos = async () => {
  ...
  rows.value = insumos.filter((insumo) => insumo.Id_finca === finca.value);
};

let crear = () => {
  router.push("/formInsumos");
};

let editar = (insumos) => {
  let id = insumos._id;
  router.push({ path: "/formInsumos", query: { id } });
};
...
const cargarResponsables = async () => {
  try {
    const data = await useInsumos.getInsumos();
    console.log(data);
...
const InsumosFiltradas = computed(() => {

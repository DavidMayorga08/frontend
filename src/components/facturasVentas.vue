...
const agregarProducto = () => {
  ...
  let productoEnInventario = options.value.find(p => p.value === productoSeleccionado.value.value);

  if (!productoEnInventario) {
    alert(`El producto "${productoSeleccionado.value.label}" no se encuentra en el inventario.`);
    return;
  }
  ...
  if (Number(cantidad.value) > stockDisponible) {
    alert(`La cantidad ingresada (${cantidad.value}) supera el stock disponible (${stockDisponible}) para "${productoSeleccionado.value.label}".`);
    return;
  }

  const isEditingExistingFactura = detallesFactura.value.length > 0;
  const productoExistente = isEditingExistingFactura
    ? detallesFactura.value.find(p => p.nombreProducto === productoSeleccionado.value.label)
    : nuevaFactura.value.productosSeleccionados.find(p => p.nombreProducto === productoSeleccionado.value.label);
  ...
  const totalGeneral = (isEditingExistingFactura ? detallesFactura.value : nuevaFactura.value.productosSeleccionados)
...
const crearFactura = async () => {
  ...
  try {
    ...
    alert("Factura creado correctamente");
...
const columns = ref([
 {
    ...
    field: (row) => row.Id_finca ? row.Id_finca.Nombre : "N/A",
  },
  {
    name: "Tipo_factura",
...
const totalPagar = computed(() => {
  const totalSinImpuesto = nuevaFactura.value.productosSeleccionados.reduce((acc, producto) => acc + producto.precio * producto.cantidad, 0);

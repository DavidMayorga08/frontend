...
let columns = [
  ...
  },
  {
    ...
    headerStyle: "font-weight: bold;",
    field: (row) => {
      let insumo = insumos.value.find((insumo) => insumo._id === row.Id_insumo);
      return insumo ? insumo.Nombre : "";
    },
  },
  {
    name: "Tipo_fertilizante",
    label: "Tipo Fertilizante",
    align: "center",
    headerStyle: "font-weight: bold;",
    field: "Tipo_fertilizante",
  },
  {
    name: "Observaciones",
    label: "Observaciones",
...
let traerFertilizantes = async () => {
  let fertilizantes = await useFertilizantes.getFertilizantes();
  insumos.value = await useInsumos.getInsumos();
  rows.value = fertilizantes.filter(
    (fertilizante) => fertilizante.Id_finca === finca.value
  );
};

let crear = () => {
  router.push("/formFertilizantes");
};

let editar = (fertilizante) => {
  let id = fertilizante._id;
  router.push({ path: "/formFertilizantes", query: { id } });
};

let finca = ref("");
const cargarResponsables = async () => {
  try {
    const data = await useFertilizantes.getFertilizantes();
    console.log(data);
    ...
    const responsableUnicos = [...new Set(data.map((u) => u.Responsable))];
    opcionesResponsable.value = responsableUnicos.map((n) => ({
      label: n,
      value: n,
    }));
  } catch (error) {
...
const FertilizanteFiltradas = computed(() => {

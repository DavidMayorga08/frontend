...
let getProveedores = async () => {
  let proveedores = await useProveedores.getProveedores();
  rows.value = proveedores.filter(
    (proveedor) => proveedor.Id_finca === finca.value
  );
};

let crear = () => {
  router.push("/formProveedores");
};

let editar = (proveedor) => {
  let id = proveedor._id;
  router.push({ name: "formProveedores", query: { id } });
};

let finca = ref("");
const filtroTipo = ref(null);

const proveedoresFiltradas = computed(() => {
  return rows.value.filter((p) => {
    const coincideEstado =
      !filtroTipo.value || p.Tipo_proveedor == filtroTipo.value;

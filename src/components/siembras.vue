<template>
  <div class="app">
    <div class="q-pa-md">
      <q-table title="Siembras" :rows="rows" :columns="columns" row-key="name">
...
let columns = [
  ...
  },
  {
    ...
    align: "center",
    headerStyle: "font-weight: bold;",
    field: (row) => row.Fecha_cosecha.split("T")[0],
  },
  {
    name: "Transplante",
    label: "Transplante",
  ...
  },
  {
    ...
    headerStyle: "font-weight: bold;",
    field: (row) => {
      let semilla = semillas.value.find(
        (semilla) => semilla._id === row.Id_semillas
      );
      return semilla ? semilla.Nombre : "";
    },
  },
  // {
...
let traerSiembras = async () => {
  ...
  semillas.value = await useSemillas.getSemillas();
  rows.value = siembras.filter((siembra) => siembra.Id_finca === finca.value);
};

let crear = () => {
  router.push("/formSiembras");
};

let editar = (siembra) => {
  let id = siembra._id;
  router.push({ path: "/formSiembras", query: { id } });
};

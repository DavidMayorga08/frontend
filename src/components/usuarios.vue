    ...
    <div class="q-pa-md">
      ...
      >
        ...
        <template v-slot:body-cell-Acciones="props">
          <q-td v-if="props.row.Rol !== 'adminFull'" :props="props">
...
let columns = [
  ...
  },
  {
    ...
    align: "center",
    headerStyle: "font-weight: bold;",
    field: (row) => row.Ultima_conexion.split("T")[0],
  },
  {
    name: "Estado",
    label: "Estado",
...
let trerUsuarios = async () => {
  let usuarios = await useUsuarios.getUsuarios();
  console.log(usuarios);
  rows.value = usuarios.filter((usuario) =>
    usuario.Fincas.some((fincaItem) => fincaItem === finca.value)
  );
};
...
let editar = (usuario) => {
  let id = usuario._id;
  router.push({ path: "/formUsuarios", query: { id } });
};

let finca = ref("");
const cargarUsuarios = async () => {
  try {
    const data = await useUsuarios.getUsuarios();
    console.log(data);
...
const rowsFiltrados = computed(() => {
  return rows.value.filter((row) => {
    const coincideNombre =
      !filtroNombre.value || row.Nombre === filtroNombre.value;
    const coincideEstado =
      !filtroEstado.value || row.Estado === filtroEstado.value;

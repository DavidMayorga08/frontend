<template>
  <div class="app">
    <div class="q-pa-md">
      <q-table
        title="Elaboración de Sustratos"
        :rows="ElaboracionFiltradas"
        :columns="columns"
        row-key="name"
      >
...
let traerElaboracionSustratos = async () => {
  ...
  rows.value = elaboracionSustratos.filter(
    (elaboracionSustrato) => elaboracionSustrato.Id_finca === finca.value
  );
};

let crear = () => {
  router.push("/formElaboracionSustratos");
};

let editar = (elaboracionSustrato) => {
  let id = elaboracionSustrato._id;
  router.push({ path: "/formElaboracionSustratos", query: { id } });
};
...
const cargarCultivos = async () => {
  try {
    ...
    const data = await useElaboracionSustratos.getElaboracionSustratos();
    console.log(data);
    ...
    const nombresUnicos = [
      ...new Map(
        data.map((item) => {
          const cultivo = cultivos.value.find((c) => c._id === item.Id_cultivo);
          return [item.Id_cultivo, cultivo?.Nombre || "Sin nombre"]; // usa .nombre o .Nombre según tu modelo
        })
      ).entries(),
    ];
    console.log(cultivos);

    opcionesTipoCultivo.value = nombresUnicos.map(([id, nombre]) => ({
      label: nombre,
      value: nombre,
    }));

    const tiposUnicos = [...new Set(data.map((u) => u.Responsable))];
    opcionesNombres.value = tiposUnicos.map((t) => ({
      label: t,
      value: t,
    }));
  } catch (error) {
...
const ElaboracionFiltradas = computed(() => {
  return rows.value.filter((p) => {
    const cultivo = cultivos.value.find((c) => c._id === p.Id_cultivo);

...
let procesosStore = useProcesosStore();
...
let titulo = ref("");
let Responsable = ref("");
let Tipo_proceso = ref("");
let Descripcion = ref("");
let Fecha_inicio = ref("");
let Fecha_fin = ref("");

if (id.value) {
  titulo.value = "Editar proceso";
} else {
  titulo.value = "Crear proceso";
}
...
function validarCampos() {
  ...
  if(!Tipo_proceso.value.trim()) {
    text.value = "El campo Tipo de Proceso es obligatorio";
    registroFallido.value = true;
    ocultar();
    return false;
  }
  if(!Descripcion.value.trim()) {
    text.value = "El campo Descripción es obligatorio";
    registroFallido.value = true;
    ocultar();
    return false;
  }
  if(!Fecha_inicio.value.trim()) {
    text.value = "El campo Fecha de Inicio es obligatorio";
    registroFallido.value = true;
    ocultar();
    return false;
  }
  if(!Fecha_fin.value.trim()) {
    text.value = "El campo Fecha de Fin es obligatorio";
    registroFallido.value = true;
    ocultar();
    return false;
  }
...
let enviar = async () => {
  ...
  if(id.value) {
    ...
    console.log(proceso);
    await procesosStore.putProceso(id.value, proceso);
    spinner.value = false;
    registroExitoso.value = true;
    text.value = "Proceso editado correctamente";
    ocultar();
  } else {
    console.log("Crear");
    spinner.value = true;
    ...
    console.log(proceso);
    await procesosStore.postProceso(proceso);
    spinner.value = false;
    registroExitoso.value = true;
    text.value = "Proceso creado correctamente";
    ocultar();
  }
}
...
onMounted(async () => {
  spinner.value = true;
  if(id.value) {
    let proceso = await procesosStore.getProceso(id.value);
    Responsable.value = proceso.Responsable;
    Tipo_proceso.value = proceso.Tipo_proceso;
    Descripcion.value = proceso.Descripcion;
    Fecha_inicio.value = proceso.Fecha_inicio.split("T")[0];
    Fecha_fin.value = proceso.Fecha_fin.split("T")[0];
  }
  spinner.value = false;
});
</script>

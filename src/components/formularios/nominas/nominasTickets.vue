      ...
      <div class="form">
        ...
        <div class="cont">
          <div class="cont_inputs">
            ...
            <select class="input" v-model="pago">
              <option
                v-for="pago in pagos"
                :key="pago._id"
                :value="pago"
          ...
          <div class="cont_inputs">
            ...
            <input type="text" class="input" v-model="detalle" />
          </div>
          <div class="cont_inputs">
            <label for="fecha_emision">Fecha de Emisión</label>
            <input type="date" class="input" v-model="fecha_emision" />
          </div>
        </div>
        <div class="cont_btn">
...
let nominasTicketsStore = useNominasTicketStore();
...
let pago = ref("");
let detalle = ref("");
let fecha_emision = ref("");

if (id.value) {
  titulo.value = "Editar ticket";
} else {
  titulo.value = "Crear ticket";
}

function validarCampos() {
  if (pago.value === "") {
    alert("El campo Pago no puede estar vacío");
    return false;
  }
  ...
  if (!fecha_emision.value) {
    alert("El campo Fecha de Emisión no puede estar vacío");
    return false;
  }
  return true;
}
...
let enviar = async () => {
  ...
  if(id.value) {
    ...
    let nominasTicket = {
      ...
      Id_pago: pago.value._id,
      Detalle: detalle.value,
      Fecha_emision: fecha_emision.value,
      Historial_modificacion: [modifica],
    }

    console.log(nominasTicket);
    let r = await nominasTicketsStore.putNominaTicket(id.value, nominasTicket);
    console.log(r);
    ...
    console.log(nominasTicket);
    await nominasTicketsStore.postNominaTicket(nominasTicket);
    spinner.value = false;
    registroExitoso.value = true;
    text.value = "Registro exitoso";
    ocultar();
...
onMounted(async () => {
  spinner.value = true;
  pagos.value = await nominasPagosStore.getNominasPagos();
  if(id.value) {
    let nominasTicket = await nominasTicketsStore.getNominaTicket(id.value);
    pago.value = pagos.value.find((pago) => pago._id === nominasTicket.Id_pago);
    detalle.value = nominasTicket.Detalle;
    fecha_emision.value = nominasTicket.Fecha_emision.split("T")[0];
  }
  spinner.value = false;
});
</script>

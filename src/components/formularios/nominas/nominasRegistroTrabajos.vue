      ...
      <div class="form">
        ...
        <div class="cont">
          ...
          <div class="part2">
            ...
            <div class="cont_inputs">
              <label for="tipoLabor">Tipo de Labor</label>
              <select class="input" v-model="tipoLabor">
                <option
                  v-for="tipo in tiposLabor"
                  :key="tipo._id"
                  :value="tipo"
                >
                  {{ tipo.Nombre }}
            ...
            <div class="cont_inputs">
              <label for="cantidad_recogida">Cantidad Recogida</label>
              <input type="number" class="input" v-model="Cantidad_recogida" />
            </div>
            <div class="cont_inputs">
              <label for="tiempo_total">Tiempo Total</label>
              <input type="number" class="input" v-model="Tiempo_total" />
            </div>
          </div>
        </div>
        <div class="cont_btn">
...
let nominasRegistroTrabajosStore = useNominasRegistroTrabajosStore();
let nominasTrabajadoresStore = useNominasTrabajadoresStore();
let parcelasStore = useParcelasStore();
let nominasTiposLaborStore = useNominasTiposLaborStore();
...
let parcela = ref("");
let tipoLabor = ref("");
let Cantidad_recogida = ref("");
let Tiempo_total = ref("");

if (id.value) {
  titulo.value = "Editar trabajo";
} else {
  titulo.value = "Crear trabajo";
}
...
function validarCampos() {
  ...
  if (!hora_inicio.value.trim()) {
    registroFallido.value = true;
    text.value = "El campo hora inicio no puede estar vacío";
    ocultar();
    return false;
  }
  if (!hora_fin.value.trim()) {
    registroFallido.value = true;
    text.value = "El campo hora fin no puede estar vacío";
    ocultar();
    return false;
  }
  if (parcela.value === "") {
    registroFallido.value = true;
    text.value = "El campo parcela no puede estar vacío";
    ocultar();
    return false;
  }
  if (tipoLabor.value === "") {
    registroFallido.value = true;
    text.value = "El campo tipo labor no puede estar vacío";
    ocultar();
    return false;
  }
  if (!String(Cantidad_recogida.value).trim()) {
    registroFallido.value = true;
    text.value = "El campo cantidad recogida no puede estar vacío";
    ocultar();
    return false;
  }
  if (!String(Tiempo_total.value).trim()) {
    registroFallido.value = true;
    text.value = "El campo tiempo total no puede estar vacío";
    ocultar();
    return false;
  }
  return true;
...
let enviar = async () => {
  ...
  if (id.value) {
    ...
    let nominasRegistroTrabajos = {
      Id_finca: id_finca.value,
      Id_trabajador: trabajador.value._id,
      Fecha: fecha.value,
      Hora_inicio: hora_inicio.value,
      Hora_fin: hora_fin.value,
      Tiempo_total: Tiempo_total.value,
      Id_parcela: parcela.value._id,
      Id_tipoLabor: tipoLabor.value._id,
      Cantidad_recogida: Cantidad_recogida.value,
      Historial_modificacion: [modifica],
    };

    console.log(nominasRegistroTrabajos);
    await nominasRegistroTrabajosStore.putNominaRegistroTrabajo(
      id.value,
      nominasRegistroTrabajos
    );
    spinner.value = false;
    registroExitoso.value = true;
    text.value = "Registro editado exitosamente";
    ocultar();
  } else {
    console.log("creando");
    spinner.value = true;
    ...
    let nominasRegistroTrabajos = {
      ...
      Id_tipoLabor: tipoLabor.value._id,
      Cantidad_recogida: Cantidad_recogida.value,
      Historial_creacion: [creacion],
    };

    console.log(nominasRegistroTrabajos);
    await nominasRegistroTrabajosStore.postNominaRegistroTrabajo(
      nominasRegistroTrabajos
    );
    spinner.value = false;
    registroExitoso.value = true;
...
onMounted(async () => {
  ...
  parcelas.value = await parcelasStore.getParcelas();
  tiposLabor.value = await nominasTiposLaborStore.getNominasTiposLabor();
  if (id.value) {
    let nominasRegistroTrabajos =
      await nominasRegistroTrabajosStore.getNominaRegistroTrabajo(id.value);
    trabajador.value = trabajadores.value.find(
      (t) => t._id === nominasRegistroTrabajos.Id_trabajador
    );
    fecha.value = nominasRegistroTrabajos.Fecha.split("T")[0];
    hora_inicio.value = nominasRegistroTrabajos.Hora_inicio;
    hora_fin.value = nominasRegistroTrabajos.Hora_fin;
    parcela.value = parcelas.value.find(
      (p) => p._id === nominasRegistroTrabajos.Id_parcela
    );
    tipoLabor.value = tiposLabor.value.find(
      (t) => t._id === nominasRegistroTrabajos.Id_tipoLabor
    );
    Cantidad_recogida.value = nominasRegistroTrabajos.Cantidad_recogida;
    Tiempo_total.value = nominasRegistroTrabajos.Tiempo_total;
  }
  spinner.value = false;
});
</script>

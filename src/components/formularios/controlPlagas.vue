  ...
  <div class="app">
        ...
        <div class="cont">
          <div class="part1">
            ...
            <div class="cont_inputs">
              ...
              <input type="text" class="input" v-model="tipo_control" />
            </div>
            <div class="cont_inputs">
          ...
          <div class="part2">
            ...
            <div class="cont_inputs">
              <label for="frecuencia_aplicacion"
                >Frecuencia de aplicacion</label
              >
              <input
                type="text"
                class="input"
                v-model="frecuencia_aplicacion"
              />
            </div>
            <div class="cont_inputs">
    ...
    <div class="cont_btn_volver">
      <q-btn
        color="primary"
        label="Volver"
        @click="router.push('/controlPlagas')"
      ></q-btn>
    </div>
...
import { useControlPlagasStore } from "../../stores/controlPlagas";
import { useCultivosStore } from "../../stores/cultivos";

let useControlPlagas = useControlPlagasStore();
...
let fecha = ref("");
let nombre = ref("");
let tipo_control = ref("");
let ingrediente_activo = ref("");
let dosis = ref("");
let frecuencia_aplicacion = ref("");
let responsable = ref("");
let estado = ref("");

if (id.value) {
  titulo.value = "Editar control de plaga";
} else {
  titulo.value = "Crear control de plaga";
}
...
function validarCampos() {
  ...
  if (!nombre.value.trim()) {
    registroFallido.value = true;
    text.value = "El campo Nombre no puede estar vacío";
    ocultar();
    return false;
  }
  if (!tipo_control.value.trim()) {
    registroFallido.value = true;
    text.value = "El campo Tipo de control no puede estar vacío";
    ocultar();
    return false;
  }
  ...
  if (!dosis.value.trim()) {
    registroFallido.value = true;
    text.value = "El campo Dosis no puede estar vacío";
    ocultar();
    return false;
  }
  if (!frecuencia_aplicacion.value.trim()) {
    registroFallido.value = true;
    text.value = "El campo Frecuencia de aplicacion no puede estar vacío";
    ocultar();
    return false;
  }
...
let enviar = async () => {
  ...
  if(id.value) {
    ...
    let controlPlaga = {
      ...
      Fecha: fecha.value,
      Nombre: nombre.value,
      Tipo_control: tipo_control.value,
      Ingrediente_activo: ingrediente_activo.value,
      Dosis: dosis.value,
      Frecuencia_aplicacion: frecuencia_aplicacion.value,
    ...
    await useControlPlagas.putControlPlaga(id.value, controlPlaga);
    spinner.value = false;
    registroExitoso.value = true;
    text.value = "Control de plaga editado exitosamente";
    ocultar();
  } else {
    console.log("Crear");
    spinner.value = true;
    ...
    console.log(controlPlaga);
    await useControlPlagas.postControlPlaga(controlPlaga);
    spinner.value = false;
    registroExitoso.value = true;
    text.value = "Control de plaga creado exitosamente";
    ocultar();
  }
}
...
onMounted(async () => {
  ...
  if(id.value) {
    let controlPlaga = await useControlPlagas.getControlPlaga(id.value);
    cultivo.value = cultivos.value.find(cultivo => cultivo._id === controlPlaga.Id_cultivo);
    fecha.value = controlPlaga.Fecha.split("T")[0];
    nombre.value = controlPlaga.Nombre;
    tipo_control.value = controlPlaga.Tipo_control;
    ingrediente_activo.value = controlPlaga.Ingrediente_activo;
    dosis.value = controlPlaga.Dosis;
    frecuencia_aplicacion.value = controlPlaga.Frecuencia_aplicacion;
    responsable.value = controlPlaga.Responsable;
    estado.value = controlPlaga.Estado;
  }
  spinner.value = false;
});
</script>

...
let exportarExcel = () => {
  const data = rows.value.map((row) => ({
    ...
    "Historial de Modificación": row.Historial_modificacion,
  }));
...
let traerGastos = async () => {
  let gastos = await useGastos.getGastos();
  console.log(gastos);
  rows.value = gastos.filter((gasto) => gasto.Id_finca === finca.value);
};

let crear = () => {
  router.push("/formGastos");
};

let editar = (gastos) => {
  let id = gastos._id;
  router.push({ path: "/formGastos", query: { id } });
};

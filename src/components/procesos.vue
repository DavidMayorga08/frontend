<template>
  <div class="app">
    <div class="q-pa-md">
      <q-table title="Procesos" :rows="rows" :columns="columns" row-key="name">
...
let columns = [
  ...
  },
  {
    ...
    align: "center",
    headerStyle: "font-weight: bold;",
    field: (row) => row.Fecha_inicio.split("T")[0],
  },
  {
    name: "Fecha_fin",
    label: "Fecha de fin",
...
let traerProcesos = async () => {
  ...
  rows.value = procesos.filter((proceso) => proceso.Id_finca === finca.value);
};

let crear = () => {
  router.push("/formProcesos");
};

let editar = (procesos) => {
  let id = procesos._id;
  router.push({ path: "/formProcesos", query: { id } });
};
